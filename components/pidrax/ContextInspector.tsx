...
export function ContextInspector() {
  ...
  const handleSave = () => {
    // Mock save - in real app would update the source
    setEditingId(null);
    setEditText('');
  };
  ...
  return (
    <div className="inspector-panel flex flex-col border-l rounded-l-xl">
      {/* Source label header - shows current source */}
      {state.source && state.source !== 'terminal' && (
        <div className="flex items-center gap-2 px-3 py-2.5 border-b shrink-0">
          ...
          <span className="text-xs font-medium capitalize">{state.source}</span>
        </div>
      )}
