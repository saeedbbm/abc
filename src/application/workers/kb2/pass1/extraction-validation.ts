...
export const extractionValidationStep: StepFunction = async (ctx) => {
  ...
  ctx.onProgress("Layer 1: Programmatic pattern scan...", 5);
  const { candidates: programmaticCandidates, docEntityMap } = runProgrammaticScan(docs, existingNames);
  ctx.onProgress(`Layer 1 done: ${programmaticCandidates.length} candidates found`, 15);
  ...
  ctx.onProgress("Layer 2: Cross-LLM gap check (GPT-4o)...", 20);
  ...
  try {
    const crossCheckModel = getCrossCheckModel();
    const startMs = Date.now();
    let usageData: { promptTokens: number; completionTokens: number } | null = null;

    const gapPrompt = `Here are all entities extracted so far:\n${entitySummary}\n\nReview these source documents and identify any entities that were MISSED or MISCATEGORIZED.\nFocus on: PRs, tickets, people, repositories, databases, and infrastructure that should be entities but aren't in the list above.`;

    const result = await structuredGenerate({
      model: crossCheckModel,
      system: "You are a quality assurance reviewer for a knowledge base entity extraction system. Your job is to find entities that the primary extraction missed. Be thorough but precise — only flag real entities, not attributes or components.",
      prompt: `${gapPrompt}\n\nDocuments to review:\n${docsText}`,
    ...
    if (usageData) {
      ...
      const cost = calculateCostUsd("gpt-4o", usageData.promptTokens, usageData.completionTokens);
      ctx.logLLMCall(stepId, "gpt-4o", gapPrompt + "\n\n" + docsText, JSON.stringify(result, null, 2), usageData.promptTokens, usageData.completionTokens, cost, durationMs);
    }
  ...
  ctx.onProgress(`Layer 2 done: ${crossLLMCandidates.length} additional candidates`, 50);

  // ---- Layer 3: Opus final judge ----
  const allCandidates = [...programmaticCandidates, ...crossLLMCandidates];

  if (allCandidates.length === 0) {
    ctx.onProgress("No candidates to validate — extraction was complete", 100);
    return {
  ...
  ctx.onProgress(`Layer 3: Opus judging ${allCandidates.length} candidates...`, 55);
  ...
  try {
    const opusModel = getReasoningModel();
    const startMs = Date.now();
    let usageData: { promptTokens: number; completionTokens: number } | null = null;
    ...
    const result = await structuredGenerate({
      model: opusModel,
      system: `You are the final judge for entity extraction validation. For each candidate:
- ADD: The entity is real and missing from the existing list. Assign the correct type from: person, team, client, repository, integration, infrastructure, cloud_resource, library, database, environment, project, ticket, pull_request, pipeline.
...
Be precise. Only ADD genuinely missing entities.`,
    ...
    if (usageData) {
      const durationMs = Date.now() - startMs;
      const cost = calculateCostUsd("claude-opus-4-6", usageData.promptTokens, usageData.completionTokens);
      ctx.logLLMCall(stepId, "claude-opus-4-6", judgePrompt, JSON.stringify(result, null, 2), usageData.promptTokens, usageData.completionTokens, cost, durationMs);
    }
  ...
  ctx.onProgress(`Layer 3 done: ${confirmed.length} confirmed, ${rejected} rejected`, 85);
  ...
  if (confirmed.length > 0) {
    const newNodes: KB2GraphNodeType[] = confirmed.map((c) => ({
      ...
      source_refs: [{
        source_type: c.source_provider as any,
        doc_id: c.source_doc_title,
        title: c.source_doc_title,
  ...
  ctx.onProgress(`Validation complete: ${existingNodes.length} → ${finalCount} entities (+${confirmed.length})`, 100);
