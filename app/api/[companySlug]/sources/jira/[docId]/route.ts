/**
 * Jira Source Document API
 * 
 * GET /api/[companySlug]/sources/jira/[docId]
 * 
 * Returns a specific Jira knowledge document by docId and projectId.
 */
...
export async function GET(
    ...
    try {
        let filter: Record<string, any> = { projectId, provider: 'jira' };

        try {
            filter._id = new ObjectId(docId);
        } catch {
            filter = { projectId, provider: 'jira', sourceId: docId };
        }
        ...
        console.error('[Sources/Jira] Error:', error);
